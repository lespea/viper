...
mod_dicts['elf'] = {'sections': '--sections', 'segments': '--segments', 'symbols': '--symbols',
                    'interp': '--interpreter', 'dynamic': '--dynamic'}
mod_dicts['email'] = {'envelope': '-e', 'attach': '-f', 'header': '-r', 'trace': '-t', 'traceall': '-T', 'spoof': '-s',
                      'all': '-a'}
mod_dicts['exif'] = {'run': ''}
mod_dicts['fuzzy'] = {'run': ''}
...
mod_dicts['pe'] = {'imports': 'imports', 'exports': 'exports', 'res': 'resources', 'imp': 'imphash',
                   'compile': 'compiletime', 'peid': 'peid', 'security': 'security', 'language': 'language',
                   'sections': 'sections', 'pehash': 'pehash'}
...
def cuckoo_submit():
    ...
    try:
        ...
        if check_file.status_code == 200:
            ...
            return '<a href="{0}/submit/status/{1}" target="_blank"> Link To Cukoo Report</a>'.format(cuckoo_web,
                                                                                                      str(cuckoo_id))
    except Exception as e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-H', '--host', help='Host to bind the API server on', default='localhost', action='store',
                        required=False)
    parser.add_argument('-p', '--port', help='Port to bind the API server on', default=9090, action='store',
                        required=False)
